...
def show_products(request):
     ...
     show = product.objects.filter( Q(availability = True) & Q(brand__istartswith= search_content) | Q(title__istartswith =  search_content))
     print(show)
...
def order_table(request,id):
    ...
    if tax['tax']:
        ...
        ret = order.objects.filter(order_user = current_user.id).values('product__product_id__image', 'order_user')
...
def order_history(request):
    current_user = request.user
    sel = order.objects.filter(order_user = current_user).values('product__product_id__image', 'order_user', 'id', 'product__selling_price', 'product__quantity', 'created_on', 'total_order_value')
...
def add_wish(request, id):
    ...
    if request.method == 'POST':
        ...
        get_price = product.objects.get(id =id)
        price = get_price.price
        a = wishlist.objects.filter(Q(product1 = product_id) & Q(user1 = request.user)).values('product1')
        print(a)
        usre_id = User.objects.get(username= request.user)
        if a :
            print('Do not save')
